...
export function ReviewQueueTable({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-background">
      <div className="grid gap-3 border-b bg-slate-50 p-4 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_180px_150px_180px]">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Search enterprise, EDO, REDO, county, sector"
        ...
        <Filter value={attention} onChange={setAttention} label="Attention">
          ...
          <option value="dqa">Open DQA findings</option>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50/70 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3">Quality and evidence</th>
              ...
              <tr key={row.submissionId} className="hover:bg-slate-50/70">
                <td className="px-4 py-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {humanize(row.track)} · {humanize(row.sector)} · {humanize(row.county)}
                  </p>
                </td>
                <td className="px-4 py-4 text-xs text-slate-700">
                  <p>
                    <span className="font-medium text-slate-900">EDO:</span> {row.edoName}
                  </p>
                  <p className="mt-1">
                    <span className="font-medium text-slate-900">REDO:</span> {row.redoName}
                  </p>
                  {row.collectorRole === "redo" && row.collectorName !== row.redoName ? (
                    <p className="mt-1 text-slate-500">Submitted by {row.collectorName}</p>
                  ) : null}
                </td>
                <td className="px-4 py-4">
                  <Badge variant="outline">{row.stage.toUpperCase()} review</Badge>
                  <p className="mt-1.5 text-xs text-slate-500">
                    {row.periodCode} · v{row.version}
                  </p>
                </td>
                <td className="px-4 py-4">
                  <div className="flex flex-wrap gap-1.5">
                    <Badge
                      variant="outline"
                      className={row.dqaErrorCount ? "border-red-200 bg-red-50 text-red-700" : ""}
                    >
                    ...
                    <Badge
                      variant="outline"
                      className={
                        row.verifiedEvidenceCount < row.evidenceCount
      ...
      {filtered.length === 0 ? (
        <p className="px-4 py-12 text-center text-sm text-slate-500">No reports match these filters.</p>
      ) : null}
    </div>
  );
}
