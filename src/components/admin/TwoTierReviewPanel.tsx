...
export function TwoTierReviewPanel({
    ...
    const [activeRole, setActiveRole] = useState<"reviewer1" | "reviewer2" | null>(null);
    ...
    async function fetchStatus() {
        try {
            // Note: Ensure this action exists and is exported correctly
    ...
    const handleModalSubmit = async (data: { score: number; notes: string; decision?: "approved" | "rejected" }) => {
        if (!activeRole) return;

        try {
            let result;
            if (activeRole === "reviewer1") {
                ...
                result = await submitReviewer2Review({
                    ...
                    score: data.score,
                    notes: data.notes,
                    finalDecision: data.decision || "approved",
                    overrideReviewer1: data.score !== reviewStatus?.reviewer1?.score,
                });
            }
    ...
    const openReview = (role: "reviewer1" | "reviewer2") => {
        setActiveRole(role);
        setModalOpen(true);
    };
    ...
    return (
        <>
            ...
            <EvaluationModal
                ...
                title={activeRole === "reviewer1" ? "Initial Review" : "Senior Review & Final Decision"}
                description={activeRole === "reviewer1" ? "Provide an initial score and detailed assessment." : "Review previous feedback and make a final determination."}
                initialScore={activeRole === "reviewer1" ? reviewStatus?.reviewer1?.score : reviewStatus?.reviewer2?.score}
                initialNotes={activeRole === "reviewer1" ? reviewStatus?.reviewer1?.notes : reviewStatus?.reviewer2?.notes}
                isFinalReviewer={activeRole === "reviewer2"}
