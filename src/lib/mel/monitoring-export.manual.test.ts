...
const overall = periodRows.find((row) => row.Track === "all");
assert.ok(overall);
assert.equal(overall.ITT_Baseline_Monthly_Profit_KES, 66100);
assert.equal(overall.ITT_Baseline_Monthly_Revenue_KES, 300000);
assert.equal(overall.ITT_Baseline_Monthly_Costs_KES, 202530);
