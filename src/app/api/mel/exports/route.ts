...
async function exportRows(type: ExportType, dataset: Awaited<ReturnType<typeof buildMelReportingDataset>> | null): Promise<Array<Record<string, string | number | boolean | null>>> {
  ...
  if (type === "monitoring") return dataset.approvedRecords.map((record) => { const baseline = record.financialBaselineSnapshot as { effectiveDate?: string; revenue?: number; costs?: number; profit?: number } | null | undefined; const comparison = record.financialComparisonSnapshot as { flags?: Array<{ code: string; source: string }> } | null | undefined; return { Submission_ID: record.submissionId, Enterprise_ID: record.businessId, Period_ID: record.periodId, Track: record.dimensions.track, County: record.dimensions.county, Sector: record.dimensions.sector, Owner_Gender: record.dimensions.ownerGender, Owner_Youth: record.dimensions.ownerYouth, Revenue_KES: record.revenue, Costs_KES: record.costs, Profit_Loss_KES: record.profitLoss, Baseline_Effective_Date: baseline?.effectiveDate ?? null, Baseline_Monthly_Revenue_KES: baseline?.revenue ?? null, Baseline_Monthly_Costs_KES: baseline?.costs ?? null, Baseline_Monthly_Profit_KES: baseline?.profit ?? null, Financial_Alert_Flags: comparison?.flags?.map((flag) => `${flag.code}:${flag.source}`).join("; ") ?? "", Financial_Change_Explanation: record.financialChangeExplanation ?? null, Finance_Types: record.financeEntries.map((entry) => entry.financeType).join("; "), Finance_Amounts_KES: record.financeEntries.map((entry) => `${entry.financeType}:${entry.amount}`).join("; "), Finance_Accessed_KES: record.financeValue, New_Market_Segments: record.newMarketSegments }; });
  if (type === "jobs") return dataset.approvedRecords.flatMap((record) => ([
