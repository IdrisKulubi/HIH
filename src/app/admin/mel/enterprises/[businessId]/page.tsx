...
export default async function MelEnterpriseDashboardPage({ params }: { params: Promise<{ businessId: string }> }) {
  ...
  const allJobs = approved.flatMap((submission) => submission.jobs);
  const directQualityJobs = allJobs
    .filter((job) => job.jobType === MEL_JOB_TYPE.directQuality || job.jobType === MEL_JOB_TYPE.legacyDirect)
    .reduce((sum, job) => sum + (job.quarterlyTotal ?? 0), 0);
  const directNonQualityJobs = allJobs
    .filter((job) => job.jobType === MEL_JOB_TYPE.directNonQuality)
    .reduce((sum, job) => sum + (job.quarterlyTotal ?? 0), 0);
  const directJobs = directQualityJobs + directNonQualityJobs;
  const indirectJobs = allJobs
    .filter((job) => job.jobType === MEL_JOB_TYPE.indirect)
    .reduce((sum, job) => sum + (job.quarterlyTotal ?? 0), 0);
  ...
  return <div className="container mx-auto space-y-7 px-4 py-8">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric icon={FileText} label="Approved periods" value={approved.length} detail={`${business.melMonitoringSubmissions.length} total submissions`} /><Metric icon={Briefcase} label="Cumulative jobs" value={directJobs + indirectJobs} detail={`Direct jobs (Quality=${directQualityJobs}, Non-quality=${directNonQualityJobs}), ${indirectJobs} indirect`} /><Metric icon={ChartLineUp} label="Active evidence" value={activeEvidence} detail="Across approved reports" /><Metric icon={Lightbulb} label="Open learning actions" value={openActions.length} detail="Requires follow-up" /></section>
